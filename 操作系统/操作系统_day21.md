...
- 异常
  - 当 $CPU$ 在执行处于用户态的程序时，发生了不可预料的异常，这时当前运行的进程会切换到处理相关异常的程序中，也就转到了内核态，最常见的是缺页异常。
    - $CPU$ 可以通过地址总线访问连接在地址总线上的所有外设。但访问是通过虚拟地址访问，而不是物理地址，所以就需要一个虚拟地址到物理地址的转换关系，并且可以设置是否可以对该物理页进行写操作。如果在某一次访问时还没有创建转换关系或者创建了转换关系但该物理页不可写时，就会发生缺页异常。
- 外围设备的中断
